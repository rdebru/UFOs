...
* Filter UFO sightings table on multiple criteria (city, state, country, and shape) in addition to date.Deploy webpage to github. As wellAs well to create a summary report based on the UFOs analysis and how search filters work.
...
# filter by date July
![filter by date July](./static/images/Data/UFOs_july.PNG)

* As we see from image above, the website has a title, an article filters on the left and whole table loaded as default on the right. Below the article onto the left under "Filter Search" we have five filters. Under the article onto the right is the table. Users should be able to filter by one, two, three, four or the filters by typing filter and clicking "enter". E.g., in the above image we are filtering by date "July”, enter the date and click enter. The table will be filtered to show only data for the month of July.
...
* If you decide you want to filter only by date and state from previous example, delete the shape filter and click enter but keep the date and state. the filtered table will be displayed. As well if you want filter only by date, keep the date and delete other filters(state and shape in this example). The filtered data for date will be displayed.
...
* As i mentioned we can update the Date filter, add two input date so uers can be able to filer the table by for different months

# filter by date June
![filter by date june](./static/images/UFOs_june.PNG)

* we can see from the image above, from the previous example user was filtering by Date, state and circle. if they decided to filter by another date for the whole table, then if they only change the date and click enter, the results displayed will also be filtered by the state and shape from the previous search. check the image above. So, the user has to delete the state and shape and hit enter for the correct filtered data to be displayed. To make it easier and we need to add a Reset button so that the user does not have to delete all filters added if new search is required. 
